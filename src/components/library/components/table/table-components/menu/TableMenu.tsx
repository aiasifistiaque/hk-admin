		...
		<Menu>
			...
			<MenuContainer>
				{data?.map((item: any, i: number) => {
					...
					if (item?.condition) {
						const docValue = doc[item?.condition?.key];
						if (Array.isArray(item?.condition?.value)) {
							if (!item?.condition?.value.includes(docValue)) {
								return null;
							}
						} else if (docValue !== item?.condition?.value) {
							return null;
						}
					}
