...
const ColoredPageTable: FC<TableProps> = ({
	table,
	layoutPath,
	children,
	schemaLoading = false,
	...
	const colorScheme = (status: string) => {
		if (status == 'pending') return 'blue.100';
		else if (status == 'completed') return 'green.100';
		else if (status == 'cancelled') return 'red.300';
		else if (status == 'order-placed') return 'purple.400';
		else return 'purple.50';
	};
...
export default ColoredPageTable;
